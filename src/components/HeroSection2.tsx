  ...
  return (
    <>
      ...
      <div className="md:hidden relative z-10 px-4 pb-4">
        <nav className="flex items-center justify-center gap-4 bg-black/20 backdrop-blur-sm rounded-lg py-2">
          <a href="#" className="font-raleway font-bold uppercase transition-colors duration-200" style={{ fontSize: '12px', letterSpacing: '1px', color: '#0e72d2' }} onMouseEnter={(e) => (e.target as HTMLElement).style.color = '#6d6e71'} onMouseLeave={(e) => (e.target as HTMLElement).style.color = '#0e72d2'}>
            Repairs
          </a>
          <a href="#" className="font-raleway font-bold uppercase transition-colors duration-200" style={{ fontSize: '12px', letterSpacing: '1px', color: '#0e72d2' }} onMouseEnter={(e) => (e.target as HTMLElement).style.color = '#6d6e71'} onMouseLeave={(e) => (e.target as HTMLElement).style.color = '#0e72d2'}>
            Services
         ...
         <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-4 md:py-8 lg:py-12">
           <div className="flex flex-col lg:flex-row gap-8 md:gap-12 h-full">
             ...
             <div className="text-white space-y-6 md:space-y-8 flex-1 flex flex-col justify-start text-center md:text-left">
               <h1 className="text-3xl md:text-4xl lg:text-5xl font-raleway font-bold leading-tight" style={{ paddingBottom: '10px' }}>
                 Lets Get It Fixed!
               </h1>
               
               <p className="font-lato font-normal text-base md:text-lg" style={{ marginTop: 0, marginBottom: 0 }}>
                 Same Day <span className="font-bold">Repairs</span>
               </p>
              
              <div className="flex flex-col items-center md:items-start gap-3 md:gap-4">
                {/* Primary CTA Button */}
                <button className="px-3 md:px-4 py-2 bg-transparent text-white text-sm md:text-base font-lato font-semibold uppercase tracking-wider border border-white rounded-full hover:bg-white hover:text-gray-800 transition-all duration-300 w-fit" style={{ marginTop: '-10px', marginBottom: '10px' }}>
                  GET STARTED
                </button>
                
                 {/* Search Bar Style Button */}
                 <button className="bg-transparent text-white text-xs md:text-sm font-lato font-normal tracking-wider border border-white rounded-full flex items-center justify-end w-fit" style={{ padding: '6px 16px' }}>
                   <span className="mr-2 text-xs md:text-sm">Search for your device here</span>
                   <svg 
                     width="14" 
                     height="14" 
                     viewBox="0 0 24 24" 
